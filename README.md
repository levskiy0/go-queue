...
Machinery logs are routed through the `slog.Logger` passed to `NewQueue`. Debug output is emitted only when both the queue debug flag and the logger handler enable it. Machinery logging is process-wide, so the first queue configures the shared logger for every queue worker in the process.
